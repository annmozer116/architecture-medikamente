...
Более подробно см. ![problems.audit.md](/Task1/problems.audit.md)

# 3. Список мер для улучшения безопаности данных
# 3.1. Cписок данных для защиты
...
# Список данных для защиты и методы защиты для компании «Медикаменте»
...
# 3.2. Механизм тегирования данных с использованием инструментов тегирования
...
# Описание категорий данных

---

# Cписок инструментов, способов и мер для обеспечения конфиденциальности в указанных потоках
...
![Registration-ToBe](Task1/diagrams/Task1-DFD-1-Registration-ToBe.png)

**Запись пациента к специалисту**
- Общий доступ только к журналу записей с минимальными идентификаторами (например, только Фамилия, Имя, дата рождения); полные ПДн — по запросу или в отдельном экране с проверкой прав.
- Блокировки или транзакции при записи в журнал (устранение коллизий при одновременной записи).
- Шифрование и контроль доступа к хранилищу журналов; аудит «кто кого записал и когда».
- Тегирование записей как PII (минимально необходимый объём) с политикой «не экспортировать без обезличивания».

![Booking-ToBe](Task1/diagrams/Task1-DFD-2-Booking-ToBe.png)

**Проведение осмотра/диагностики**
- Изоляция медданных: отдельная схема/хранилище или политики доступа (только врач к своим приёмам/назначениям или по явному разрешению).
- Шифрование в покое и при передаче (TLS при доступе через веб/API); для файлов AS-IS — шифрование каталогов `patients/pacient-FIO-BD` и `laboratory/DATE`.
- RBAC: доступ к диагнозам и результатам анализов только у врачей и уполномоченного медперсонала; ресепшен/колл-центр без доступа к PHI.
- Аудит CRUD по меддокументам; алерты на массовое чтение или экспорт.
- Тегирование PHI на все поля с диагнозами и результатами; проверка при интеграциях (например, с лабораторией).

![Examination-ToBe](Task1/diagrams/Task1-DFD-3-Examination-ToBe.png)

**Оплата услуг**
- Шифрование данных в `bills/` и при передаче в 1С; защищённый канал (TLS, VPN или защищённый API) между кассой/приложением и 1С.
- Разделение прав: кассир — только операции по оплате; доступ к истории платежей пациента — по роли и с аудитом.
- Не хранить платёжные данные вместе с медданными; теги FIN на платёжные сущности.
- Аудит операций с платежами; при использовании ККМ — соответствие регламентам и защита канала до 1С.

![Payment-ToBe](Task1/diagrams/Task1-DFD-4-Payment-ToBe.png)

**Сдача анализов**
- Передача в лабораторию только минимально необходимых данных (ФИО, телефон, почта) по защищённому каналу (API с аутентификацией и TLS вместо незащищённой почты где возможно).
- Результаты из лаборатории — приём только в защищённый контур (защищённая почта или API), сохранение в зашифрованное хранилище с тегом PHI.
- Доступ к папке `laboratory/DATE` и к результатам анализов — только у врачей и медсестёр по ролям; аудит доступа.
- Тегирование результатов анализов как PHI; при передаче в BI/аналитику — только обезличенные или агрегированные данные.

![LabTests-ToBe](Task1/diagrams/Task1-DFD-5-LabTests-ToBe.png)


**Общие меры для всех потоков**
- DLP (контроль каналов: почта, USB, веб) для рабочих мест с доступом к PII/PHI.
- Регулярный пересмотр прав доступа и каталога данных; обучение сотрудников.
- Политика хранения и удаления/обезличивания с привязкой к тегам и целям обработки.